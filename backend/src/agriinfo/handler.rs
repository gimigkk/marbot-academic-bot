...
use super::models::{NewAgriinfoTask, AgriinfoAIExtraction, AgriinfoTask};
use super::crud::create_agriinfo_task;
use super::prompts::build_agriinfo_extraction_prompt;
...
fn format_agriinfo_tasks(tasks: Vec<AgriinfoTask>) -> String {
    use chrono::Datelike;
    ...
    if active_tasks.is_empty() {
        return "*[Daftar Tugas Agriinformatics]*\n\n📭 Belum ada tugas untuk periode ini.".to_string();
    }

    let mut response = String::from("*[Daftar Tugas Agriinformatics]*\n\n");
...
async fn call_ai_extraction(prompt: &str) -> Result<AgriinfoAIExtraction, String> {
    ...
    if response.status().is_success() {
        ...
        // Parse string JSON tersebut ke struct AgriinfoAIExtraction
        let extraction: AgriinfoAIExtraction = serde_json::from_str(content)
...
pub async fn process_agriinfo_message(
    pool: &PgPool,
    message_body: &str,
    chat_id: &str,
    logger: &JobLogger,
    ...
    if lower_body.starts_with("#tugas") || lower_body.starts_with("#tgs") {
        logger.log("📋 Command #tugas terdeteksi untuk Agriinformatics!");
        match super::crud::get_upcoming_agriinfo_tasks(pool).await {
            Ok(tasks) => {
                let msg = format_agriinfo_tasks(tasks);
                let _ = send_reply(chat_id, &msg).await;
                logger.set_status(crate::tui::state::JobStatus::Completed);
            }
            Err(e) => {
                logger.log(&format!("❌ Gagal mengambil tugas Agriinfo: {}", e));
                let _ = send_reply(chat_id, "❌ Gagal mengambil daftar tugas Agriinfo.").await;
    ...
    if lower_body.starts_with("#help") {
        logger.log("❓ Command #help terdeteksi untuk Agriinformatics!");
        let help_msg = "*[Bantuan Agriinformatics]*\n\n\
            *Perintah yang tersedia:*\n\
            - #tugas / #tgs — lihat daftar tugas Agriinfo aktif\n\
            ...
            *Menambah Tugas Baru:*\n\
            ...
            Contoh: _#marbot jangan lupa rapat koordinasi agriinfo besok siang_\n\
    ...
    if lower_body.starts_with("#delete") {
        logger.log("🗑️ Command #delete terdeteksi untuk Agriinformatics!");
        ...
        match super::crud::get_upcoming_agriinfo_tasks(pool).await {
            Ok(tasks) => {
                ...
                match super::crud::delete_agriinfo_task(pool, target_task.id).await {
                    ...
                    Err(e) => {
                        logger.log(&format!("❌ Gagal hapus tugas Agriinfo: {}", e));
    ...
    if lower_body.starts_with("#update") {
        logger.log("🔄 Command #update terdeteksi untuk Agriinformatics!");
                ...
                match call_ai_extraction(&prompt).await {
                    Ok(ai_result) => {
                        if ai_result.is_task {
                            ...
                            let updated_task = NewAgriinfoTask {
                                nama_tugas: new_nama.clone(),
                                deadline: parsed_deadline,
                            };

                            // 3. Simpan ke DB
                            match super::crud::update_agriinfo_task(pool, target_task.id, updated_task).await {
                                Ok(saved) => {
                                ...
                                Err(e) => {
                                    logger.log(&format!("❌ Gagal update tugas Agriinfo ke DB: {}", e));
    ...
    if !lower_body.contains("#marbot") {
        logger.log("💬 Pesan Agriinfo diabaikan (tidak mengandung #marbot).");
        logger.set_status(crate::tui::state::JobStatus::Completed);
        return;
    }

    logger.log("🎪 Command #marbot terdeteksi! Memproses pesan Agriinformatics...");

    // 2. BERSIHKAN PESAN & BUILD PROMPT
    let clean_message = message_body.replace("(?i)#marbot", "").replace("#marbot", "").trim().to_string();
    let prompt = build_agriinfo_extraction_prompt(&clean_message);
    ...
    if ai_result.is_task {
        ...
        if tasks_to_process.is_empty() && ai_result.nama_tugas.is_some() {
            tasks_to_process.push(super::models::AgriinfoAITaskData {
        ...
        for task_data in tasks_to_process {
            ...
            let new_task = NewAgriinfoTask {
                nama_tugas,
                deadline,
            };

            // 5. SIMPAN KE DATABASE
            match create_agriinfo_task(pool, new_task).await {
                Ok(saved) => {
                    saved_tasks.push(saved);
                }
                Err(e) => {
                    logger.log(&format!("❌ Gagal menyimpan tugas Agriinfo ke DB: {}", e));
                }
            }
        }

        if saved_tasks.is_empty() {
            let _ = send_reply(chat_id, "❌ Gagal menyimpan agenda Agriinfo ke database internal.").await;
            logger.set_status(crate::tui::state::JobStatus::Failed);
        } else {
            let mut success_msg = String::from("✅ *Agenda Agriinfo Berhasil Dicatat*\n\n");
...
pub async fn send_daily_reminder(pool: &PgPool, logger: &JobLogger) -> Result<(), String> {
    ...
    if agriinfo_group_id.is_empty() {
        logger.log("⚠️ KADIV_AGRIINFOMATICS_GROUP kosong, skip Agriinfo reminder");
        return Ok(());
    }
    ...
    match super::crud::get_upcoming_agriinfo_tasks(pool).await {
        Ok(tasks) => {
            ...
            if active_tasks.is_empty() {
                logger.log("📭 Tidak ada tugas Agriinfo aktif, skip reminder");
                return Ok(());
            }

            let msg = format_agriinfo_tasks(active_tasks);
            let final_msg = msg.replacen(
                "*[Daftar Tugas Agriinformatics]*\n\n", 
                "🌄*[Pengingat Tugas Agriinformatics]*\n_Selamat pagi!_\n\n", 
                1
            );
            
            let _ = send_reply(&agriinfo_group_id, &final_msg).await;
            logger.log("✅ Reminder Agriinfo harian berhasil dikirim");
            Ok(())
        }
        Err(e) => {
            let err_msg = format!("Gagal mengambil tugas Agriinfo untuk reminder: {}", e);
            logger.log(&err_msg);
            Err(err_msg)
        }
    }
...
pub async fn check_urgent_agriinfo_deadlines(pool: &PgPool, logger: &JobLogger) -> Result<(), String> {
    ...
    logger.log(&format!(
        "🚨 Menemukan {} tugas Agriinfo deadline < 1 jam",
        rows.len()
    ));
    ...
    if agriinfo_group_id.is_empty() {
        logger.log("⚠️ KADIV_AGRIINFOMATICS_GROUP tidak diset, skip reminder Agriinfo");
        return Ok(());
    }

    let mut message = String::new();
    message.push_str("*[REMINDER AGRIINFORMATICS! H-1 JAM]*\n\n");
    ...
    send_reply(&agriinfo_group_id, &message).await.map_err(|e| e.to_string())?;

    for row in rows {
        sqlx::query("UPDATE agriinformatics SET reminder_1h_sent = TRUE WHERE id = $1")
            .bind(row.id)
            .execute(pool)
            .await
            .map_err(|e| e.to_string())?;
    ...
    logger.log("✅ Reminder urgent Agriinfo berhasil dikirim.");
    Ok(())
}
